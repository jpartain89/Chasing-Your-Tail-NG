...
class SurveillanceDetector:
    ...
    def analyze_surveillance_patterns(self) -> List[SuspiciousDevice]:
        ...
        for mac, appearances in self.device_history.items():
            ...
            if (
                persistence_score > self.thresholds["min_persistence_score"]
    ...
    def _calculate_persistence_score(
        ...
        if appearance_rate >= 0.5:  # Appeared at least every 2 hours
            ...
            reasons.append(
                f"Appeared {len(appearances)} times over {time_span_hours:.1f} hours"
            )
            ...
            if unique_locations > 1:
                reasons.append(
                    f"Followed across {unique_locations} different locations"
                )
    ...
    def _generate_analysis_statistics(self) -> Dict:
        ...
        analysis_duration_hours = (
            analysis_duration / 3600 if analysis_duration > 0 else 0
        )
        ...
        persistence_rate = (
            len(persistent_devices) / unique_devices if unique_devices > 0 else 0
        )
        ...
        multi_location_rate = (
            multi_location_devices / unique_devices if unique_devices > 0 else 0
        )
            ...
            if len(appearances) >= 3:
                ...
                if intervals:
                    ...
                    if variance < 3600:  # Low variance = clustered timing
                        clustered_devices += 1
        temporal_clustering = (
            clustered_devices / unique_devices if unique_devices > 0 else 0
        )
        ...
        off_hours_rate = (
            off_hours_appearances / total_appearances if total_appearances > 0 else 0
        )
        ...
        probe_anomaly_rate = (
            anomalous_devices / unique_devices if unique_devices > 0 else 0
        )
    ...
    def _format_detailed_device_analysis(
        self, device: SuspiciousDevice, persistence_level: str
    ) -> str:
        ...
        lines.append("")
        lines.append(
            "*A MAC address is like a unique fingerprint for each wireless device (phone, laptop, etc.)*"
        )
        lines.append("")
        lines.append("**📊 Persistence Analysis:**")
        lines.append(f"- **Pattern Type:** {persistence_level} FREQUENCY")
        lines.append(
            f"- **Persistence Score:** {device.persistence_score:.3f}/1.000 *(Higher = More Suspicious)*"
        )
        lines.append(
            f"- **Confidence:** {min(device.persistence_score * 100, 95):.1f}% *(How sure we are this is suspicious)*"
        )
        lines.append(
            f"- **Pattern Analysis:** {'📊 High-frequency appearance pattern' if persistence_level == 'CRITICAL' else '📈 Notable appearance pattern' if persistence_level == 'HIGH' else '📋 Low-frequency pattern'}"
        )
        lines.append("")
        ...
        lines.append("")
        lines.append(
            f"- **Total Surveillance Period:** {duration_hours:.1f} hours ({duration.days} days)"
        )
        lines.append(
            f"- **First Time Spotted:** {device.first_seen.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        lines.append(
            f"- **Most Recent Sighting:** {device.last_seen.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        lines.append(f"- **Total Appearances:** {device.total_appearances} times")
        lines.append(
            f"- **How Often It Appears:** {device.total_appearances / max(duration_hours, 1):.2f} times per hour"
        )
        lines.append("")
        if device.total_appearances > 10:
            lines.append(
                "  📊 **Analysis:** This device appears very frequently, which is unusual for normal devices."
            )
        elif device.total_appearances > 5:
            lines.append(
                "  📊 **Analysis:** This device appears regularly, worth monitoring."
            )
        else:
            lines.append(
                "  📊 **Analysis:** Low appearance count, may not be actively tracking."
            )
        lines.append("")
        ...
        if len(device.locations_seen) > 1:
            lines.append(
                f"- **Following Behavior:** ✅ **CONFIRMED** - This device has appeared at multiple locations"
            )
            lines.append(
                "  🚨 **This is a major red flag - normal devices don't follow you around!**"
            )
        else:
            lines.append(f"- **Following Behavior:** ❌ Only seen at one location")
            lines.append(
                "  ℹ️ **This could be a local device, but monitor for movement**"
            )
        lines.append("")
        ...
        recent_appearances = sorted(
            device.appearances, key=lambda a: a.timestamp, reverse=True
        )[:10]
        for appearance in recent_appearances:
            dt = datetime.fromtimestamp(appearance.timestamp)
            ssids = (
                ", ".join(appearance.ssids_probed[:2])
                if appearance.ssids_probed
                else "No probes"
            )
            lines.append(
                f"- `{dt.strftime('%Y-%m-%d %H:%M:%S')}` | Location: `{appearance.location_id}` | SSIDs: {ssids}"
            )

        if len(device.appearances) > 10:
            lines.append(
                f"- *... and {len(device.appearances) - 10} additional appearances*"
            )
        lines.append("")
        ...
        lines.append(
            "- 📊 **Data Analysis**: This device showed repeated appearances in your wireless environment"
        )
        lines.append(
            "- 🔍 **Consider**: This pattern might be worth noting or monitoring"
        )
        lines.append(
            "- 📝 **Documentation**: You could keep a log of when/where this device appears"
        )
        lines.append(
            "- 🤔 **Context**: Remember this could be a neighbor, business device, or normal wireless traffic"
        )
        lines.append(
            "- ⚖️ **Disclaimer**: These are statistical patterns only - not definitive proof of surveillance"
        )
        lines.append("")
        lines.append("---")
        lines.append("")
    ...
    def _analyze_temporal_patterns(
        self, suspicious_devices: List[SuspiciousDevice]
    ) -> List[str]:
        ...
        for device in suspicious_devices:
            hours = [
                datetime.fromtimestamp(a.timestamp).hour for a in device.appearances
            ]
        ...
        if work_hour_devices > 0:
            patterns.append(
                f"**{work_hour_devices} devices** show work-hours-only activity (9 AM - 5 PM) - possible workplace surveillance"
            )

        if off_hour_devices > 0:
            patterns.append(
                f"**{off_hour_devices} devices** show off-hours activity (10 PM - 6 AM) - possible stalking behavior"
            )

        if regular_interval_devices > 0:
            patterns.append(
                f"**{regular_interval_devices} devices** appear at highly regular intervals - possible automated tracking"
            )
        ...
        if weekday_heavy > 0:
            patterns.append(
                f"**{weekday_heavy} devices** appear primarily on weekdays - consistent with workplace/professional surveillance"
            )

        if weekend_heavy > 0:
            patterns.append(
                f"**{weekend_heavy} devices** appear primarily on weekends - may indicate personal stalking"
            )

        if not patterns:
            patterns.append(
                "No significant temporal patterns detected across suspicious devices"
            )
    ...
    def _analyze_geographic_patterns(
        ...
        # Multi-location tracking analysis
        multi_location_count = len(
            [d for d in suspicious_devices if len(d.locations_seen) > 1]
        )
        if multi_location_count > 0:
            patterns.append(
                f"**{multi_location_count} devices** confirmed following across multiple locations"
            )
        ...
        hotspot_locations = [
            loc for loc, count in location_frequency.items() if count > 1
        ]
        if hotspot_locations:
            patterns.append(
                f"**Surveillance hotspots detected:** {', '.join(hotspot_locations)} - multiple suspicious devices at these locations"
            )
        ...
        for device in suspicious_devices:
            if len(device.appearances) > 1:
                sorted_appearances = sorted(
                    device.appearances, key=lambda a: a.timestamp
                )
                for i in range(1, len(sorted_appearances)):
                    ...
                    time_diff = (
                        sorted_appearances[i].timestamp
                        - sorted_appearances[i - 1].timestamp
                    )
        ...
        if quick_followers > 0:
            patterns.append(
                f"**{quick_followers} devices** show rapid location transitions (< 30 minutes) - active following behavior"
            )
    ...
    def _analyze_device_correlations(
        self, suspicious_devices: List[SuspiciousDevice]
    ) -> List[str]:
        ...
        for i, device1 in enumerate(suspicious_devices):
            for j, device2 in enumerate(suspicious_devices[i + 1 :], i + 1):
                ...
                common_locations = set(device1.locations_seen) & set(
                    device2.locations_seen
                )
                if len(common_locations) > 1:
                    correlations.append(
                        f"**{device1.mac}** and **{device2.mac}** both appear at: {', '.join(common_locations)}"
                    )
                ...
                if temporal_matches > 2:
                    correlations.append(
                        f"**{device1.mac}** and **{device2.mac}** appear together {temporal_matches} times - possible coordinated surveillance"
                    )
    ...
    def generate_surveillance_report(self, output_file: str) -> str:
        ...
        report.append("")
        report.append(
            "This analysis examines wireless devices around you to detect potential surveillance or stalking. Here's how it works:"
        )
        report.append("")
        report.append("**🔍 What We Monitor:**")
        report.append(
            "- Wireless devices (phones, laptops, tracking devices) that appear near you"
        )
        report.append(
            "- Whether the same devices show up repeatedly or follow you to different locations"
        )
        report.append(
            "- Unusual patterns that might indicate someone is tracking your movements"
        )
        report.append("")
        report.append("**🎯 What We Look For:**")
        report.append("- **Persistence:** Devices that keep appearing over time")
        report.append(
            "- **Following:** Devices that show up at your home, work, and other locations"
        )
        report.append(
            "- **Suspicious Timing:** Devices active during unusual hours or at regular intervals"
        )
        report.append(
            "- **Tracking Behavior:** Patterns that suggest intentional surveillance"
        )
        report.append("")
        report.append(
            "**✅ Your Safety:** If no persistent_devices are found, your wireless environment appears normal and safe."
        )
        report.append(
            "**⚠️ Threats Detected:** If suspicious devices are identified, we'll explain exactly what's concerning and what you should do."
        )
        report.append("")
        report.append("---")
        report.append("")
        ...
        report.append(
            f"**Report Generated:** {datetime.now().strftime('%A, %B %d, %Y at %H:%M:%S')}"
        )
        ...
        if not suspicious_devices:
            ...
            high_persistence_count = len(
                [d for d in suspicious_devices if d.persistence_score > 0.8]
            )
            medium_threat_count = len(
                [d for d in suspicious_devices if 0.6 <= d.persistence_score <= 0.8]
            )
        ...
        report.append(
            f"**Monitoring Period:** {stats['analysis_duration_hours']:.1f} hours"
        )
        ...
        report.append("")
        report.append(
            "*This dashboard analyzes your wireless environment for suspicious patterns that could indicate surveillance or stalking.*"
        )
        report.append("")
        report.append("| Metric | Value | Risk Indicator | What This Means |")
        report.append("|--------|-------|----------------|-----------------|")
        report.append(
            f"| **Device Persistence Rate** | {stats['persistence_rate']:.1%} | {'🔴 High' if stats['persistence_rate'] > 0.3 else '🟡 Medium' if stats['persistence_rate'] > 0.15 else '🟢 Normal'} | Percentage of devices that appear repeatedly over time. High rates may indicate surveillance devices staying near you. |"
        )
        report.append(
            f"| **Multi-Location Tracking** | {stats['multi_location_rate']:.1%} | {'🔴 Critical' if stats['multi_location_rate'] > 0.2 else '🟡 Elevated' if stats['multi_location_rate'] > 0.1 else '🟢 Normal'} | Percentage of devices that follow you across different locations. This is a strong indicator of stalking/surveillance. |"
        )
        report.append(
            f"| **Analysis Time Period** | {stats['analysis_duration_hours']:.1f} hours | {'🟢 Comprehensive' if stats['analysis_duration_hours'] > 12 else '🟡 Moderate' if stats['analysis_duration_hours'] > 6 else '🔴 Limited'} | How long the monitoring period covered. Longer periods provide more reliable results. |"
        )
        report.append(
            f"| **Data Quality Score** | {min(95, int(stats['detection_accuracy'] * 100))}% | {'🟢 Excellent' if stats['detection_accuracy'] > 0.9 else '🟡 Good' if stats['detection_accuracy'] > 0.8 else '🔴 Poor'} | Reliability of the analysis based on data completeness and GPS accuracy. |"
        )
        report.append("")
        ...
        report.append("")
        report.append(
            "**Device Persistence Rate:** In a normal environment, most devices (phones, laptops) appear briefly and then leave. If many devices keep appearing repeatedly, it could indicate:"
        )
        ...
        report.append("")
        report.append(
            "**Multi-Location Tracking:** This is often considered the most serious indicator. If the same devices appear at your home, work, and other locations, it might suggest:"
        )
        ...
        report.append("")
        report.append(
            "**Analysis Time Period:** Shows how long your monitoring covered. Longer periods provide more reliable results:"
        )
        ...
        report.append("")
        report.append(
            "**Data Quality Score:** Indicates how reliable the analysis results are based on:"
        )
        ...
        if suspicious_devices:
            ...
            report.append("")
            report.append(
                "*The following devices showed repeated wireless activity patterns in your environment.*"
            )
            report.append("")
            ...
            report.append("")
            report.append(
                "Each device gets a **Persistence Score** from 0.0 to 1.0 based on suspicious behaviors:"
            )
            report.append("")
            report.append(
                "**🟢 Normal (0.0-0.5):** Typical behavior - device appears briefly, doesn't follow you"
            )
            report.append(
                "**🟡 Suspicious (0.6-0.7):** Some concerning patterns - appears multiple times or locations"
            )
            report.append(
                "**⚠️ High Threat (0.8-0.9):** Strong surveillance indicators - follows you, appears regularly"
            )
            report.append(
                "**🚨 Critical (0.9-1.0):** Almost certain surveillance - tracks you across locations with suspicious timing"
            )
            report.append("")
            report.append("**What Makes a Device Suspicious:**")
            report.append(
                "- **Appears repeatedly** at the same location over hours/days"
            )
            report.append(
                "- **Follows you** to different locations (home, work, store)"
            )
            report.append("- **Regular timing** - shows up at predictable times")
            report.append("- **Night activity** - appears during unusual hours")
            report.append(
                "- **Suspicious scanning** - searches for networks with concerning names"
            )
            report.append("")

            # Threat classification
            critical_persistent_devices = [
                d for d in suspicious_devices if d.persistence_score > 0.9
            ]
            high_persistences = [
                d for d in suspicious_devices if 0.8 <= d.persistence_score <= 0.9
            ]
            medium_persistent_devices = [
                d for d in suspicious_devices if 0.6 <= d.persistence_score < 0.8
            ]
            low_persistent_devices = [
                d for d in suspicious_devices if d.persistence_score < 0.6
            ]

            if critical_persistent_devices:
                report.append("### 📊 VERY HIGH PERSISTENCE DEVICES (Score > 0.9)")
                report.append(
                    "*These devices appeared very frequently in your wireless environment*"
                )
                report.append("")
                for device in critical_persistent_devices:
                    report.append(
                        self._format_detailed_device_analysis(device, "CRITICAL")
                    )

            if high_persistences:
                report.append("### 📈 HIGH PERSISTENCE DEVICES (Score 0.8-0.9)")
                report.append(
                    "*These devices appeared frequently and might be worth noting*"
                )
                report.append("")
            ...
            if medium_persistent_devices:
                ...
                for device in medium_persistent_devices:
                    report.append(
                        self._format_detailed_device_analysis(device, "MEDIUM")
                    )
            ...
            if correlations:
                report.append(
                    "*Devices that appear together may indicate coordinated surveillance*"
                )
                report.append("")
            ...
            report.append("")
            report.append(
                "**Analysis Result:** No suspicious surveillance patterns identified in the monitored environment."
            )
            report.append("")
            report.append("**Assessment Details:**")
            report.append(
                f"- **{stats['unique_devices']:,} unique devices** analyzed across **{stats['unique_locations']} locations**"
            )
            report.append(
                f"- **{stats['total_appearances']:,} device appearances** processed over **{stats['analysis_duration_hours']:.1f} hours**"
            )
            report.append(
                "- All device behaviors fall within normal operational parameters"
            )
        ...
        report.append("")
        report.append(
            "*Based on your analysis results, here are specific actions you can take to protect yourself.*"
        )
        report.append("")
        ...
        if suspicious_devices:
            ...
            if high_persistence:
                ...
                report.append(
                    "**ℹ️ Some devices showed high-frequency appearances - here are some general privacy tips:**"
                )
                report.append("")
                report.append("#### 1. 📱 Consider Protecting Your Devices")
                report.append(
                    "**MAC Address Randomization** *(Could make your devices harder to track)*:"
                )
                report.append(
                    "- **iPhone/iPad:** Settings → Wi-Fi → Tap info (i) next to network → Consider enabling 'Private Address'"
                )
                report.append(
                    "- **Android:** Settings → Wi-Fi → Advanced → Consider enabling 'Use randomized MAC'"
                )
                report.append(
                    "- **Windows:** Settings → Network & Internet → Wi-Fi → Manage known networks → Properties → Consider enabling 'Use random hardware addresses'"
                )
                report.append(
                    "- **Mac:** System Preferences → Network → Wi-Fi → Advanced → Consider enabling 'Use private Wi-Fi address'"
                )
                report.append("")
                report.append("**When You Might Consider Disabling Wi-Fi:**")
                report.append(
                    "- You could turn off Wi-Fi when walking around in public"
                )
                report.append(
                    "- You might use cellular data instead when you suspect someone is following"
                )
                report.append(
                    "- You could consider a Faraday bag (signal-blocking pouch) for your phone in high-risk situations"
                )
                report.append("")
                ...
                report.append(
                    "- You could visit different stores/restaurants than usual"
                )
                report.append(
                    "- If possible, you might consider staying with friends/family temporarily"
                )
                report.append("")
                report.append(
                    "**Why This Could Help:** Surveillance often relies on predictable patterns. By changing your routine, you could potentially make it harder for someone to track you."
                )
                report.append("")
                ...
                report.append(
                    "- You might take photos of suspicious people or vehicles"
                )
                report.append(
                    "- You could keep a log of when/where you notice anything unusual"
                )
                report.append("")
                report.append("**Contacting Authorities (Your Choice):**")
                report.append(
                    "- **If you feel in immediate danger:** You could call 911"
                )
                report.append(
                    "- **For stalking/harassment:** You might contact local police non-emergency line"
                )
                report.append(
                    "- **For cybersecurity help:** You could consider consulting a security professional"
                )
                report.append(
                    "- **Legal protection:** You might research restraining orders if you know who's involved"
                )
                report.append("")

        report.append("### 🔒 LONG-TERM PRIVACY PROTECTION")
        report.append(
            "*These steps help protect you from future surveillance attempts.*"
        )
        report.append("")
        ...
        report.append(
            "- You could trust your instincts - if something feels wrong, it might be"
        )
        ...
        report.append(
            "- You could run CYT analysis regularly if you're concerned about surveillance"
        )
        report.append(
            "- You might pay attention to devices that appear in multiple locations"
        )
        report.append(
            "- You could share reports with law enforcement if patterns emerge"
        )
        ...
        report.append(
            "- Devices broadcast unique identifiers (MAC addresses) when searching for Wi-Fi networks"
        )
        report.append(
            "- **Modern phones (iOS 14+, Android 10+) randomize these addresses** to protect privacy"
        )
        report.append(
            "- **Older devices or those with randomization disabled** still reveal their true MAC address"
        )
        report.append(
            "- Surveillance equipment could record these identifiers to track device movements"
        )
        report.append(
            "- This tool detects patterns where the same identifiers appear repeatedly or across locations"
        )
        report.append(
            "- **Note:** Randomized MAC addresses make tracking much harder but don't eliminate all risks"
        )
        report.append("")
        ...
        report.append("```")
        report.append(
            f"Minimum Appearances Threshold: {self.thresholds['min_appearances']}"
        )
        report.append(
            f"Minimum Time Span: {self.thresholds['min_time_span_hours']:.1f} hours"
        )
        report.append(
            f"Minimum Persistence Score: {self.thresholds['min_persistence_score']}"
        )
        report.append("```")
        report.append("")
        ...
        report.append("")
        report.append(
            "*This report was generated by the CYT Advanced Surveillance Detection System.*"
        )
        report.append(
            "*For technical support or threat intelligence inquiries, contact your security administrator.*"
        )
        report.append("")
        report.append(f"**Report ID:** CYT-{datetime.now().strftime('%Y%m%d%H%M%S')}")
        report.append(
            f"**Classification:** CONFIDENTIAL - Personal Security Intelligence"
        )
        ...
        try:
            ...
            cmd = [
                ...
                "--self-contained",
                "--metadata",
                f"title=CYT Surveillance Detection Report",
                "--css",
                "/dev/stdin",
            ]
...
def load_appearances_from_kismet(
    db_path: str, detector: SurveillanceDetector, location_id: str = "unknown"
) -> int:
