...
class SecureCYTMonitor:
    ...
    def __init__(
        ...
        self.ignore_list = set(
            mac.upper() for mac in ignore_list
    ...
    def _initialize_mac_lists(
        self, db: SecureKismetDB, boundaries: Dict[str, float]
    ) -> None:
    ...
    def _filter_ssids(self, ssid_list: List[str]) -> Set[str]:
        ...
        return {
            ssid for ssid in ssid_list if ssid and ssid not in self.ssid_ignore_list
        }
    ...
    def _log_rotation_stats(self) -> None:
        ...
        # Log to file
        self.log_file.write(
            f"{len(self.fifteen_twenty_min_ago_macs)} MACs moved to the 15-20 Min list\n"
        )
        self.log_file.write(
            f"{len(self.ten_fifteen_min_ago_macs)} MACs moved to the 10-15 Min list\n"
        )
        self.log_file.write(
            f"{len(self.five_ten_min_ago_macs)} MACs moved to the 5 to 10 mins ago list\n"
        )

        print(
            f"{len(self.fifteen_twenty_min_ago_ssids)} Probed SSIDs moved to the 15 to 20 mins ago list"
        )
        print(
            f"{len(self.ten_fifteen_min_ago_ssids)} Probed SSIDs moved to the 10 to 15 mins ago list"
        )
        print(
            f"{len(self.five_ten_min_ago_ssids)} Probed SSIDs moved to the 5 to 10 mins ago list"
        )
