...
class SurveillanceAnalyzer:
    ...
    def analyze_kismet_data(
        self, kismet_db_path: str = None, gps_data: list = None
    ) -> dict:
        ...
        if not kismet_db_path:
            ...
            recent_db_files = [
                db for db in all_db_files if os.path.getmtime(db) >= hours_24_ago
            ]
            recent_db_files = sorted(
                recent_db_files, key=os.path.getmtime, reverse=True
            )

            if not recent_db_files:
                print(
                    f"⚠️ No databases found from past {self.analysis_window_hours} hours, using most recent"
                )
                kismet_db_path = max(all_db_files, key=os.path.getmtime)
            else:
                print(
                    f"📊 Found {len(recent_db_files)} databases from past {self.analysis_window_hours} hours:"
                )
                total_gps_coords = 0
                for db_file in recent_db_files:
                    try:
                        ...
                        cursor = conn.cursor()
                        cursor.execute(
                            "SELECT COUNT(*) FROM devices WHERE avg_lat != 0 AND avg_lon != 0"
                        )
                        gps_count = cursor.fetchone()[0]
                        conn.close()
                        print(
                            f"   📁 {os.path.basename(db_file)}: {gps_count} GPS locations"
                        )
                ...
                print(
                    f"🛰️ Total GPS coordinates across all databases: {total_gps_coords}"
                )
        ...
        db_files_to_process = (
            kismet_db_path if isinstance(kismet_db_path, list) else [kismet_db_path]
        )
            ...
            try:
                ...
                for db_file in db_files_to_process:
                    try:
                        ...
                        if db_coords:
                            print(
                                f"   📁 {os.path.basename(db_file)}: {len(db_coords)} GPS locations"
                            )
                ...
                if all_gps_coords:
                    ...
                    for lat, lon, timestamp in all_gps_coords:
                        ...
                        if prev_lat and prev_lon:
                            ...
                            distance = (
                                math.sqrt((lat - prev_lat) ** 2 + (lon - prev_lon) ** 2)
                                * 111000
                            )  # rough meters
                            if distance < 50:
                                continue
                        ...
                        location_id = self.gps_tracker.add_gps_reading(
                            lat, lon, location_name=location_name
                        )
                    ...
                    print(
                        "⚠️ No GPS coordinates found in any Kismet database - using single location mode"
                    )
        ...
        if gps_data:
            ...
            for db_file in db_files_to_process:
                ...
                print(
                    f"   📁 {os.path.basename(db_file)}: {db_count} device appearances"
                )
            ...
            for db_file in db_files_to_process:
                db_count = load_appearances_from_kismet(
                    db_file, self.detector, "unknown_location"
                )
                print(
                    f"   📁 {os.path.basename(db_file)}: {db_count} device appearances"
                )
        ...
        if suspicious_devices:
            ...
            for i, device in enumerate(suspicious_devices[:5], 1):
                ...
                print(
                    f"     Appearances: {device.total_appearances}, Locations: {len(device.locations_seen)}"
                )
        ...
        surv_reports_dir = self.config.get("paths", {}).get(
        ...
        if gps_data:
            ...
            self.kml_exporter.generate_kml(
                self.gps_tracker, suspicious_devices, kml_file
            )
    ...
    def _load_appearances_with_gps(self, db_path: str, location_id: str) -> int:
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                ...
                if (
                    hasattr(self.gps_tracker, "location_sessions")
                    and self.gps_tracker.location_sessions
                ):
...
def main():
    ...
    parser.add_argument(
        "--demo", action="store_true", help="Run demo analysis with simulated GPS data"
    )
    parser.add_argument(
        "--kismet-db", type=str, help="Path to specific Kismet database file"
    )
    parser.add_argument("--gps-file", type=str, help="JSON file with GPS coordinates")
    parser.add_argument(
        "--stalking-only",
        action="store_true",
        help="Focus analysis on stalking detection",
    )
    parser.add_argument("--output-json", type=str, help="Export results to JSON file")
    parser.add_argument(
        "--min-threat",
        type=float,
        default=0.5,
            ...
            if stalking_only:
                ...
                if stalking_devices:
                    ...
                    for device in stalking_devices:
                        print(
                            f"   ⚠️ {device.mac} (Stalking Score: {device.stalking_score:.2f})"
                        )
