...
class CLISetupWizard:
    ...
    def run(self) -> bool:
        ...
        if self.config.is_setup_complete():
            if not self.get_yes_no(
                "Setup has already been completed. Run setup again?", False
            ):
            ...
            try:
                ...
                if 0 <= idx < len(found_paths):
                    ...
                    custom_path = self.get_input(
                        "Enter Kismet database path pattern", current_path
                    )
                    self.config.config["paths"]["kismet_logs"] = custom_path
            except ValueError:
                print(
                    "  ⚠️ Invalid input. Using previous or default Kismet database path."
                )
        else:
    ...
    def _setup_wigle(self):
        ...
        if len(api_name) > 100 or any(
            c in api_name for c in ["<", ">", '"', "'", "&", ";"]
        ):
    ...
    def _setup_geographic(self):
        ...
        if self.get_yes_no("Use default boundaries (Arizona/Southwest US)?", True):
            ...
            try:
                lat_min = float(
                    self.get_input(
                        "Minimum Latitude", str(current.get("lat_min", 31.3))
                    )
                )
                lat_max = float(
                    self.get_input(
                        "Maximum Latitude", str(current.get("lat_max", 37.0))
                    )
                )
                lon_min = float(
                    self.get_input(
                        "Minimum Longitude", str(current.get("lon_min", -114.8))
                    )
                )
                lon_max = float(
                    self.get_input(
                        "Maximum Longitude", str(current.get("lon_max", -109.0))
                    )
                )
                ...
                if not (-90 <= lat_min <= 90 and -90 <= lat_max <= 90):
                    print(
                        "⚠️ Latitude values must be between -90 and 90. Using defaults."
                    )
                elif not (-180 <= lon_min <= 180 and -180 <= lon_max <= 180):
                    print(
                        "⚠️ Longitude values must be between -180 and 180. Using defaults."
                    )
                else:
    ...
    def _setup_timing(self):
        ...
        current_interval = self.config.config.get("timing", {}).get(
            "check_interval", 60
        )

        if self.get_yes_no(
            f"Use default check interval ({current_interval} seconds)?", True
        ):
            pass
        else:
            try:
                interval = int(
                    self.get_input("Check interval (seconds)", str(current_interval))
                )
                if interval >= 10:
    ...
    class GUISetupWizard:
        ...
        def _create_wizard_content(self, parent):
            ...
            self.step_label = tk.Label(
                self.main_frame, text="", font=("Arial", 12), fg="#888888", bg="#1a1a1a"
            )
        ...
        def _create_label(
            self, parent, text: str, font_size: int = 12, color: str = "#cccccc"
        ):
        ...
        def _create_kismet_step(self):
            ...
            self.kismet_entry = self._create_entry(
                entry_frame, self.config.config["paths"]["kismet_logs"]
            )
        ...
        def _create_search_step(self):
            ...
            entries = [
                ...
                (
                    "Minimum Longitude:",
                    "lon_min",
                    current_search.get("lon_min", -114.8),
                ),
                (
                    "Maximum Longitude:",
                    "lon_max",
                    current_search.get("lon_max", -109.0),
                ),
            ]
                        ...
                        try:
                            ...
                            with test_mode_for_setup():
                                ...
                                cred_manager.store_credential(
                                    "wigle", "encoded_token", encoded_token
                                )
                        except Exception as e:
                            messagebox.showwarning(
