...
ignore_list = open(
    pathlib.Path("./ignore_lists") / config["paths"]["ignore_lists"]["mac"], "w"
)
...
def grab_all_probes(con):
    ...
    for row in rows:
        ...
        if "dot11.probedssid.ssid" in str(row):
            ssid_probed_for = raw_device_json["dot11.device"][
                "dot11.device.last_probed_ssid_record"
            ][
                "dot11.probedssid.ssid"
            ]  ### Grabbed SSID Probed for
            if ssid_probed_for == "":
                pass
            else:
...
# Fix - write to ignore_lists directory
ignore_list_ssid = open(
    pathlib.Path("./ignore_lists") / config["paths"]["ignore_lists"]["ssid"], "w"
)
