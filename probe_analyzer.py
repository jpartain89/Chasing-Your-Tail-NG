...
class ProbeAnalyzer:
    def __init__(self, log_dir=None, local_only=True, days_back=14):
        ...
        if not self.wigle_api_key and not local_only:
            print(
                "⚠️  No WiGLE API token found in secure storage. Use --local for offline analysis."
            )
    ...
    def parse_log_file(self, log_file):
        ...
        timestamp_pattern = re.compile(
            r"Current Time: (\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})"
        )
                ...
                if len(filename_parts) >= 3:
                    ...
                    if len(date_str) == 6:
                        ...
                        if file_date >= cutoff_date:
                            ...
                            print(
                                f"- Skipping old file: {log_file.name} ({file_date.strftime('%Y-%m-%d')})"
                            )
    ...
    def query_wigle(self, ssid):
        ...
        if self.local_only:
            ...
            if all(
                search_config.get(k) is not None
    ...
    def analyze_probes(self):
        ...
        for i, (ssid, timestamps) in enumerate(self.probes.items(), 1):
            ...
            result = {
                ...
                "wigle_data": (
                    self.query_wigle(ssid)
                    if (self.wigle_api_key and not self.local_only)
                    else None
                ),
            }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze probe requests and query WiGLE"
    )
    parser.add_argument(
        "--wigle",
        action="store_true",
...
def run_probe_analysis(
    use_wigle: bool = False, days_back: int = 14, all_logs: bool = False
):
    ...
    with redirect_stdout(output_buffer):
        if (
            len(glob.glob(str(pathlib.Path(config["paths"]["log_dir"]) / "cyt_log_*")))
            == 0
        ):
        ...
        for result in results:
            ...
            if duration.total_seconds() > 0:
                ...
                print(
                    f"Average frequency: {result['count'] / duration.total_seconds():.2f} probes/second"
                )
                ...
                if "error" in result["wigle_data"]:
                    ...
                    if locations:
                        ...
                        for loc in locations[:3]:
                            print(
                                f"- Lat: {loc.get('trilat')}, Lon: {loc.get('trilong')}"
                            )
