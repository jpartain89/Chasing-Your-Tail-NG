...
<Document>
    ...
    def generate_kml(
        ...
        output_file: str = "cyt_analysis.kml",
    ) -> str:
        ...
        content_parts.append(
            "<description>Geographic locations where surveillance detection occurred</description>"
        )
            ...
            if surveillance_devices:
                for device in surveillance_devices:
                    if session.session_id in device.locations_seen:
                        location_persistence_score = max(
                            location_persistence_score, device.persistence_score
                        )
    ...
    <Placemark>
        ...
        if surveillance_devices:
            ...
            critical_devices = [
                d for d in surveillance_devices if d.persistence_score > 0.9
            ]
            high_devices = [
                d for d in surveillance_devices if 0.8 <= d.persistence_score <= 0.9
            ]
            medium_devices = [
                d for d in surveillance_devices if 0.6 <= d.persistence_score < 0.8
            ]
            ...
            if critical_devices:
                ...
                content_parts.append(
                    "<description>Devices with highest activity patterns</description>"
                )
                self._add_device_tracking_folder(
                    content_parts, critical_devices, gps_tracker, "CRITICAL"
                )
                content_parts.append("</Folder>")
            ...
            if high_devices:
                ...
                content_parts.append(
                    "<description>Devices with notable activity patterns</description>"
                )
                self._add_device_tracking_folder(
                    content_parts, high_devices, gps_tracker, "HIGH"
                )
                content_parts.append("</Folder>")
            ...
            if medium_devices:
                ...
                content_parts.append(
                    "<description>Devices with some activity patterns</description>"
                )
                self._add_device_tracking_folder(
                    content_parts, medium_devices, gps_tracker, "MEDIUM"
                )
                content_parts.append("</Folder>")
            ...
            content_parts.append(
                "<description>Advanced surveillance pattern visualization</description>"
            )

            # Add heat map style analysis
            self._add_surveillance_heatmap(
                content_parts, surveillance_devices, gps_tracker
            )

            # Add temporal analysis tracks
            self._add_temporal_analysis_tracks(
                content_parts, surveillance_devices, gps_tracker
            )
        ...
        logger.info(
            f"📊 Includes {len(gps_tracker.location_sessions)} locations, {len(surveillance_devices) if surveillance_devices else 0} surveillance devices"
        )
        return kml_output
    ...
    def _format_enhanced_device_list(
        self, all_devices: List[str], suspicious_devices: List
    ) -> str:
        ...
        for device in suspicious_devices:
            persistence_emoji = (
                "🚨"
                if device.persistence_score > 0.8
                else "⚠️" if device.persistence_score > 0.6 else "🟡"
            )
    ...
    def _format_location_persistence_analysis(self, suspicious_devices: List) -> str:
        ...
        for device in suspicious_devices:
            html += (
                f"<li><b>{device.mac}</b> (Score: {device.persistence_score:.2f})<ul>"
            )
        ...
        for device in devices:
            if device.mac in multi_location_devices:
                ...
                for session in sorted(
                    gps_tracker.location_sessions, key=lambda s: s.start_time
                ):
                    if session.session_id in locations:
                        path_coordinates.append(
                            f"{session.location.longitude},{session.location.latitude},0"
                        )
                        location_times.append(
                            datetime.fromtimestamp(session.start_time)
                        )
        ...
        <LineString>
            <coordinates>
                ...
                for session in gps_tracker.location_sessions:
                    if (
                        session.session_id in locations
                        and device.mac in session.devices_seen
                    ):
                        # Find device appearances at this location
                        appearances_here = [
                            a
                            for a in device.appearances
                            if a.location_id == session.session_id
                        ]
    ...
    def _add_surveillance_heatmap(
        self,
        content_parts: List[str],
        surveillance_devices: List,
        gps_tracker: GPSTracker,
    ) -> None:
        ...
        for location, scores in location_intensity.items():
            ...
            if session:
                ...
                radius = min(
                    max(device_count * 50, 100), 500
                )  # Scale radius by device count
    ...
    def _add_temporal_analysis_tracks(
        self,
        content_parts: List[str],
        surveillance_devices: List,
        gps_tracker: GPSTracker,
    ...
    def _generate_circle_coordinates(
        self, center_lon: float, center_lat: float, radius_meters: float
    ) -> str:
