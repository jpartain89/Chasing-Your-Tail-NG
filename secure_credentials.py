...
class SecureCredentialManager:
    ...
    def _get_master_password(self) -> str:
        ...
        try:
            ...
            print(
                "⚠️  Non-interactive environment detected. Using environment variables."
            )
            print(
                "Set CYT_MASTER_PASSWORD environment variable or use CYT_TEST_MODE=true for testing"
            )
...
def secure_config_loader(config_path: str = "config.json") -> Dict[str, Any]:
    ...
    if "api_keys" in config:
        ...
        if any(
            "token" in str(api_keys).lower() or "key" in str(api_keys).lower()
            for key in api_keys.values()
            if isinstance(key, dict)
        ):

            logger.warning(
                "Found API keys in config file - initiating secure migration"
            )
